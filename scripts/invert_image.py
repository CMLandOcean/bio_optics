...
from bio_optics.inversion import lmfit_engine as model
...
def invert_chunk(chunk,
    ...
    for i in np.arange(chunk.shape[0]):
        inv = model.invert(params,
                           ...
                           forward_func=albert_mobley_3C.forward,
                           weights=weights,
         ...
         rio.open(('/').join([output_dir, args.input_file.split('/')[-1] + '_Rrs']), 'w', **Rrs_profile) as dst_Rrs:
            ...
            for idx, wind in dataset_reader.block_windows():
                ...
                if data.shape[0] > 0:
                    ...
                    out_fwd[:,~np.isnan(rrs).all(axis=0)] = np.array([albert_mobley_3C.forward(i.params,
                                                        wavelengths[wl_mask],
                                                        a_i_spec_res=a_i_spec_res,
                                                        a_w_res=a_w_res,
                                                        b_phy_norm_res=b_phy_norm_res,
                    ...
                    glint = np.array([surface_reflectance.forward(i.params,
                                                        wavelengths[wl_mask],
                                                        E0_res=E0_res,
                                                        a_ox_res=a_ox_res,
                                                        a_wv_res=a_wv_res,
