...
def train_test_split(X, Y, abs_size=2000, returnSampleIDs=False):
    ...
    if returnSampleIDs:
        return X_train, X_test, Y_train, Y_test, ID, IDbool
    else:
...
def NN_tensorflow_training_ModelRrs2IOP(d, NNdict, NNtraining_metadata, outpath = '', epochs=1000, batch_size=500,
    ...
    ###
    # ForwardNN load
    forwardNN_HEREON = tf.keras.models.load_model(NNtraining_metadata['forwardNN_path'])
    ...
    if NNdict['activation_function_output_layer'] == 'softmax':
        ...
        #     return a(test, rhow_pred)
        def minimise_rhow_AND_iop(IOP_pred, training_input, IOP_orig):
            rhow_pred = forwardNN_HEREON(IOP_pred)
            test = training_input
            a = keras.losses.MeanSquaredError()
            return a(test, rhow_pred) + a(IOP_orig, IOP_pred)
        ...
        if epoch >= epoch_start:
            ...
            for step, (Y_batch, X_batch) in enumerate(train_dataset_fw.take(1), 1):
                ...
                with tf.GradientTape() as tape_fw:
                    ...
                    loss_value_ = loss_fn(logits_, X_batch, Y_batch)
