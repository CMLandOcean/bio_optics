...
def setup_model_layers_tensorflow(model_dict):
    ...
    for i in range(len(nodes_layer)):
        if i == 0:
            if activation_layer[i] == 'leakyRelu':
                x = layers.Dense(nodes_layer[i])(inputLayer)
                x = layers.LeakyReLU()(x)
            else:
                x = layers.Dense(nodes_layer[i], activation=activation_layer[i])(inputLayer)
        else:
            if activation_layer[i] == 'leakyRelu':
                x = layers.Dense(nodes_layer[i])(x)
                x = layers.LeakyReLU()(x)
            else:
    ...
    if model_dict['activation_function_output_layer'] == '':
        outputs = layers.Dense(model_dict['Noutput'])(x)
        # model_list.append(Dense(model_dict['Noutput'])(last_layer))
    else:
        print(model_dict['activation_function_output_layer'])
        outputs = layers.Dense(model_dict['Noutput'], activation=model_dict['activation_function_output_layer'])(x)
...
def NN_tensorflow_training_ModelRrs2IOP(d, NNdict, NNtraining_metadata, outpath = '', epochs=1000, batch_size=500,
    ...
    ###
    # Initialise the forward model HEREON
    ...
    params.add('C_1', value=0, min=1e-10, max=1000, vary=False)  # green
    ...
    if NNdict['activation_function_output_layer'] == 'softmax':
        ...
        def minimise_rhow(IOP_pred, training_input):
            # set parameters for hereon.forward
            # IOP_pred_r = transform_NorthSea_IOPs(IOP_pred, reverse=True)
            chunk_params = params.copy()
            shape = (IOP_pred.shape[0], len(wavelengths))
            # rhow_pred = tf.constant(np.zeros(shape))
            rhow_pred = np.zeros(shape)
            # rhow_pred = keras.KerasTensor(shape=(IOP_pred.shape[0], len(wavelengths)))
            for i in range(IOP_pred.shape[0]):
                # set fixed IOP values, for variable: 'C_0', 'C_2', 'C_5', 'C_6', 'C_7', 'C_Y', 'C_ism', 'L_fl_lambda0', 'b_ratio_md', 'b_ratio_bd', 'S_cdom'
                chunk_params.add('C_0', value=transform_NorthSea_IOPs_single(IOP_pred[i, 0], 'C_0', reverse=True), vary=False)  # brown
                chunk_params.add('C_2', value=transform_NorthSea_IOPs_single(IOP_pred[i, 1], 'C_2', reverse=True), vary=False)  # brown
                chunk_params.add('C_5', value=transform_NorthSea_IOPs_single(IOP_pred[i, 2], 'C_5', reverse=True), vary=False)  # brown
                chunk_params.add('C_6', value=transform_NorthSea_IOPs_single(IOP_pred[i, 3], 'C_6', reverse=True), vary=False)  # brown
                chunk_params.add('C_7', value=transform_NorthSea_IOPs_single(IOP_pred[i, 4], 'C_7', reverse=True), vary=False)  # brown
                chunk_params.add('C_Y', value=transform_NorthSea_IOPs_single(IOP_pred[i, 5], 'C_Y', reverse=True), vary=False)  # brown
                chunk_params.add('C_ism', value=transform_NorthSea_IOPs_single(IOP_pred[i, 6], 'C_ism', reverse=True), vary=False)  # brown
                chunk_params.add('L_fl_lambda0', value=transform_NorthSea_IOPs_single(IOP_pred[i, 7], 'L_fl_lambda0', reverse=True), vary=False)  # brown
                chunk_params.add('b_ratio_md', value=transform_NorthSea_IOPs_single(IOP_pred[i, 8], 'b_ratio_md', reverse=True), vary=False)  # brown
                chunk_params.add('b_ratio_bd', value=transform_NorthSea_IOPs_single(IOP_pred[i, 9], 'b_ratio_bd', reverse=True), vary=False)  # brown
                chunk_params.add('S_cdom', value=transform_NorthSea_IOPs_single(IOP_pred[i, 10], 'S_cdom', reverse=True), vary=False)  # brown
                # a = np.zeros(shape)
                rhow_pred[i,:] = hereon.forward(parameters=chunk_params,
            ...
            test = training_input
            rhow_pred_T = tf.convert_to_tensor(rhow_pred)
            a = keras.losses.MeanSquaredError()
            return a(test, rhow_pred_T)

        loss_fn = minimise_rhow
        # model_.compile(optimizer=optimizer, loss="mean_squared_error")
        model_.compile(optimizer=optimizer, loss=loss_fn)
        ...
        if epoch >= epoch_start:
            ...
            for step, (Y_batch, X_batch) in enumerate(train_dataset_fw.take(1), 1):
                ...
                with tf.GradientTape() as tape_fw:
                    ...
                    loss_value_ = loss_fn(logits_, X_batch)
    ...
    outfname = NNname + 'batch'+ str(batch_size) + '_epoch' + str(epochs+epoch_start) +'_loss'+ str(np.round(loss_[-1], ndigits))
    model_.save(outpath + outfname + '.h5')
    ...
    fig, axes = plt.subplots(1, 1, figsize=(8, 4))
    axes.semilogy(loss_, 'g-')
    fig.savefig(outpath + 'loss_'+outfname+'.png', dpi=200)
    plt.close()
    # plt.show()
...
NNtraining_metadata = {
    ...
    'architectures': [[80, 80, 80, 80]],  # [80, 80, 80]
