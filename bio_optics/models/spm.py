...
def petus(Rrs, wavelengths, a=12450, b=666.1, c=0.48, lambda0=645):
    """
...
def gaa(Rrs, wavelengths, lambda1=486, lambda2=551, lambda3=671, lambda4=745, lambda5=862, a1=20.43, a2=2.15, c0=0.04, c1=1.17, c2=0.4, c3=14.86):
    """
    ...
    band4 = Rrs[find_closest(wavelengths, lambda4)[1]]
    band5 = Rrs[find_closest(wavelengths, lambda5)[1]]
...
def dsa(Rrs, wavelengths, lambda1=671, lambda2=551, a=1.25, b=1.11):
    """
...
def jiang(Rrs, wavelengths, lambda1=443, lambda2=490, lambda3=560, lambda4=620, lambda5=665, lambda6=754, lambda7=865):
    ...
    bb_w = resample_bb_w(wavelengths=wl, fresh=False)

    ## Step 1: Compute optical water type classification
    water_type = owt.jiang(Rrs, wavelengths)

    ## Step 2: Model preparation 
    # Compute Rrs and u for selected wavelengths
    rrs = above2below(Rrs[idx])
    u = (-0.089 + np.sqrt(0.089**2 + 4 * 0.125 * rrs)) / (2 * 0.125)   
    
    # Type 1:
    x = np.log((rrs[lambdas.index(443)] + rrs[lambdas.index(490)]) / (rrs[lambdas.index(560)] + 5 * (rrs[lambdas.index(665)]/rrs[lambdas.index(490)]) * rrs[lambdas.index(665)])) 
    a_560 = a_w[lambdas.index(560)] + 10**(-1.146 - 1.366 * x - 0.469 * x**2)                                                                   
    tss_type1 = 94.607 * ((u[lambdas.index(560)] * a_560) / (1 - u[lambdas.index(560)]) - bb_w[lambdas.index(560)])

    # Type 2: [!!! changed the scalar in a_665 to 0.039 instead of 0.39 !!!]
    a_665 = a_w[lambdas.index(665)] + 0.039 * (Rrs[idx[lambdas.index(665)]] / (Rrs[idx[lambdas.index(443)]] + Rrs[idx[lambdas.index(490)]]))**1.14
    tss_type2 = 114.012 * ((u[lambdas.index(665)] * a_665) / (1 - u[lambdas.index(665)]) - bb_w[lambdas.index(665)])
    
    # Type 3:
    tss_type3 = 137.665 * ((u[lambdas.index(754)] * a_w[lambdas.index(754)]) / (1 - u[lambdas.index(754)]) - bb_w[lambdas.index(754)])

    # Type 4:
    tss_type4 = 166.168 * ((u[lambdas.index(865)] * a_w[lambdas.index(865)]) / (1 - u[lambdas.index(865)]) - bb_w[lambdas.index(865)])
