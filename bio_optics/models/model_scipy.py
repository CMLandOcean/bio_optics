...
class ModelScipy():
    ...
    def __init__(self, params, 
        ...
        self.a_i_spec_res=a_i_spec_res
        self.a_w_res=a_w_res
        self.a_Y_N_res=a_Y_N_res
        self.a_NAP_N_res=a_NAP_N_res
        self.b_phy_norm_res=b_phy_norm_res
        self.b_bw_res=b_bw_res
        self.b_X_norm_res=b_X_norm_res
        self.b_Mie_norm_res=b_Mie_norm_res
        self.b_b_res=b_b_res
        self.R_i_b_res=R_i_b_res
        self.da_W_div_dT_res=da_W_div_dT_res
        self.a_oz_res=a_oz_res
        self.a_ox_res=a_ox_res
        self.a_wv_res=a_wv_res
        self.E_0_res=E_0_res
        self.E_dd_res=E_dd_res
        self.E_dsa_res=E_dsa_res
        self.E_dsr_res=E_dsr_res
        self.E_d_res=E_d_res
        self.Ls_Ed=Ls_Ed
        self.n2_res=n2_res
        self.ones = np.ones_like(wavelengths)
        self.R_rs = None
    ...
    def forward(self, params=None):
        ...
        if len(self.Ls_Ed) == 0:
            Ls_Ed = np.zeros_like(self.wavelengths)
        else:
            Ls_Ed = self.Ls_Ed            
        ...
        if len(self.a_res) == 0:
            a_sim = absorption.a(C_0=parameters["C_0"], C_1=parameters["C_1"], C_2=parameters["C_2"], C_3=parameters["C_3"], C_4=parameters["C_4"], C_5=parameters["C_5"], 
                                ...
                                a_w_res=self.a_w_res,
                                da_W_div_dT_res=self.da_W_div_dT_res, 
                                a_i_spec_res=self.a_i_spec_res, 
                                a_Y_N_res=self.a_Y_N_res,
                                a_NAP_N_res=self.a_NAP_N_res)
        else:
            a_sim = self.a_res
        
        if len(self.b_b_res) == 0:
            b_b_sim = backscattering.b_b(C_X=parameters["C_X"], C_Mie=parameters["C_Mie"], C_phy=np.sum([parameters["C_0"], parameters["C_1"], parameters["C_2"], parameters["C_3"], parameters["C_4"], parameters["C_5"]]), wavelengths=self.wavelengths, 
                                ...
                                n=parameters["n"],
                                b_bw_res=self.b_bw_res, 
                                b_phy_norm_res=self.b_phy_norm_res, 
                                b_X_norm_res=self.b_X_norm_res, 
                                b_Mie_norm_res=self.b_Mie_norm_res)
        else:
            b_b_sim = self.b_b_res

        Rrsb = bottom_reflectance.R_rs_b(parameters["f_0"], parameters["f_1"], parameters["f_2"], parameters["f_3"], parameters["f_4"], parameters["f_5"], B_0=parameters["B_0"], B_1=parameters["B_1"], B_2=parameters["B_2"], B_3=parameters["B_3"], B_4=parameters["B_4"], B_5=parameters["B_5"], wavelengths=self.wavelengths, R_i_b_res=self.R_i_b_res)
        ...
        if parameters["fit_surface"].value:
            if len(self.E_dd_res) == 0:
                E_dd  = downwelling_irradiance.E_dd(self.wavelengths, parameters["theta_sun"], parameters["P"], parameters["AM"], parameters["RH"], parameters["H_oz"], parameters["WV"], parameters["alpha"], parameters["beta"], self.E_0_res, self.a_oz_res, self.a_ox_res, self.a_wv_res, self.E_dd_res)
            else:
                E_dd = self.E_dd_res

            if len(self.E_dsa_res) == 0:
                E_dsa = downwelling_irradiance.E_dsa(self.wavelengths, parameters["theta_sun"], parameters["P"], parameters["AM"], parameters["RH"], parameters["H_oz"], parameters["WV"], parameters["alpha"], parameters["beta"], self.E_0_res, self.a_oz_res, self.a_ox_res, self.a_wv_res, self.E_dsa_res)
            else:
                E_dsa = self.E_dsa_res

            if len(self.E_dsr_res) == 0:
                E_dsr = downwelling_irradiance.E_dsr(self.wavelengths, parameters["theta_sun"], parameters["P"], parameters["AM"], parameters["RH"], parameters["H_oz"], parameters["WV"], parameters["alpha"], parameters["beta"], self.E_0_res, self.a_oz_res, self.a_ox_res, self.a_wv_res, self.E_dsr_res)
            else:
                E_dsr = self.E_dsr_res

            if len(self.E_ds_res) == 0:
                E_ds = downwelling_irradiance.E_ds(E_dsr, E_dsa)
            else:
                E_ds = self.E_ds_res

            if len(self.E_d_res) == 0:
    ...
    def forward_glint(self):
        ...
        if "rho_L" in self.parameters:
            rho_L = self.parameters["rho_L"].value
        else:
            rho_L = air_water.fresnel(self.parameters["theta_view"], n1=self.parameters["n1"], n2=n2)

        if len(self.Ls_Ed) == 0:
            Ls_Ed = np.zeros_like(self.wavelengths)

        if len(self.E_dd_res) == 0:
            E_dd  = downwelling_irradiance.E_dd(self.wavelengths, self.parameters["theta_sun"], self.parameters["P"], self.parameters["AM"], self.parameters["RH"], self.parameters["H_oz"],self. parameters["WV"], self.parameters["alpha"], self.parameters["beta"], self.E_0_res, self.a_oz_res, self.a_ox_res, self.a_wv_res, self.E_dd_res)
        else:
            E_dd = self.E_0_res, self.a_oz_res, self.a_ox_res, self.a_wv_res, self.E_dsa_resE_dd_res

        if len(self.E_dsa_res) == 0:
            E_dsa = downwelling_irradiance.E_dsa(self.wavelengths, self.parameters["theta_sun"], self.parameters["P"], self.parameters["AM"], self.parameters["RH"], self.parameters["H_oz"], self.parameters["WV"], self.parameters["alpha"], self.parameters["beta"], self.E_0_res, self.a_oz_res, self.a_ox_res, self.a_wv_res, self.E_dsa_res)
        else:
            E_dsa = self.E_dsa_res

        if len(self.E_dsr_res) == 0:
            E_dsr = downwelling_irradiance.E_dsr(self.wavelengths, self.parameters["theta_sun"], self.parameters["P"], self.parameters["AM"], self.parameters["RH"], self.parameters["H_oz"], self.parameters["WV"], self.parameters["alpha"], self.parameters["beta"], self.E_0_res, self.a_oz_res, self.a_ox_res, self.a_wv_res, self.E_dsa_res)
        else:
            E_dsr = self.E_dsr_res
        ...
        if len(self.E_d_res) == 0:
            E_d = downwelling_irradiance.E_d(E_dd, E_ds, self.parameters["f_dd"], self.parameters["f_ds"])
        else:
            E_d = self.E_d_res

        L_s = sky_radiance.L_s(self.parameters["f_dd"], self.parameters["g_dd"], E_dd, self.parameters["f_ds"], self.parameters["g_dsr"], E_dsr, self.parameters["g_dsa"], E_dsa)

        R_rs_surface = surface.R_rs_surf(L_s, E_d, rho_L, self.parameters["d_r"])

        R_rs_surface += air_water.fresnel(self.parameters['theta_view'], n2=n2) * Ls_Ed
    ...
    def dfun(self):
        ...
        for param in self.parameters.keys():
            if self.parameters[param].vary or self.parameters[param].expr:
        ...
        ctsp = np.cos(air_water.snell(self.parameters["theta_sun"], n1=self.parameters["n1"],  n2=n2))  #cos of theta_sun_prime. theta_sun_prime = snell(theta_sun, n1, n2)
        ctvp = np.cos(air_water.snell(self.parameters["theta_view"], n1=self.parameters["n1"], n2=n2))

        if len(self.a_res) == 0:
            a_sim = absorption.a(C_0=self.parameters["C_0"], C_1=self.parameters["C_1"], C_2=self.parameters["C_2"], C_3=self.parameters["C_3"], C_4=self.parameters["C_4"], C_5=self.parameters["C_5"], 
                                C_Y=self.parameters["C_Y"], C_X=self.parameters["C_X"], C_Mie=self.parameters["C_Mie"], S=self.parameters["S"], 
                                S_NAP=self.parameters["S_NAP"], 
                                a_NAP_spec_lambda_0=self.parameters["a_NAP_spec_lambda_0"],
                                lambda_0=self.parameters["lambda_0"],
                                K=self.parameters["K"],
                                wavelengths=self.wavelengths,
                                T_W=self.parameters["T_W"],
                                T_W_0=self.parameters["T_W_0"],
                                a_w_res=self.a_w_res,
                                da_W_div_dT_res=self.da_W_div_dT_res, 
                                a_i_spec_res=self.a_i_spec_res, 
                                a_Y_N_res=self.a_Y_N_res,
        ...
        if len(self.b_b_res) == 0:
            b_b_sim = backscattering.b_b(C_X=self.parameters["C_X"], C_Mie=self.parameters["C_Mie"], C_phy=np.sum([self.parameters["C_0"], self.self.parameters["C_1"], self.self.parameters["C_2"], self.self.parameters["C_3"], self.self.parameters["C_4"], self.self.parameters["C_5"]]), wavelengths=self.wavelengths, 
                                fresh=self.parameters["fresh"],
                                b_bphy_spec=self.parameters["b_bphy_spec"],
                                b_bMie_spec=self.parameters["b_bMie_spec"],
                                b_bX_spec=self.parameters["b_bX_spec"],
                                b_bX_norm_factor=self.parameters["b_bX_norm_factor"],
        ...
        Rrsb = bottom_reflectance.R_rs_b(self.parameters["f_0"], self.self.parameters["f_1"], self.self.parameters["f_2"], self.self.parameters["f_3"], self.self.parameters["f_4"], self.self.parameters["f_5"], B_0=self.parameters["B_0"], B_1=self.parameters["B_1"], B_2=self.parameters["B_2"], B_3=self.parameters["B_3"], B_4=self.parameters["B_4"], B_5=self.parameters["B_5"], wavelengths=self.wavelengths, R_i_b_res=self.R_i_b_res)
        ...
        Kd =  attenuation.K_d(a=a_sim, b_b=b_b_sim, cos_t_sun_p=ctsp, kappa_0=self.parameters["kappa_0"])

        kuW = attenuation.k_uW(a=a_sim, b_b=b_b_sim, omega_b=ob, cos_t_sun_p=ctsp, cos_t_view_p=ctvp)

        kuB = attenuation.k_uB(a=a_sim, b_b=b_b_sim, omega_b=ob, cos_t_sun_p=ctsp, cos_t_view_p=ctvp)

        R_rs_water = air_water.below2above(water_alg.r_rs_shallow(r_rs_deep=rrsd, K_d=Kd, k_uW=kuW, zB=self.parameters["zB"], R_rs_b=Rrsb, k_uB=kuB)) # zeta & gamma

        if len(self.E_dd_res) == 0:
            E_dd  = downwelling_irradiance.E_dd(self.wavelengths, self.self.parameters["theta_sun"], self.self.parameters["P"], self.self.parameters["AM"], self.self.parameters["RH"], self.self.parameters["H_oz"], self.self.parameters["WV"], self.self.parameters["alpha"], self.self.parameters["beta"], self.E_0_res, self.a_oz_res, self.a_ox_res, self.a_wv_res, self.E_dd_res)
        else:
            E_dd = self.E_dd_res

        if len(self.E_dsa_res) == 0:
            E_dsa = downwelling_irradiance.E_dsa(self.wavelengths, self.self.parameters["theta_sun"], self.self.parameters["P"], self.self.parameters["AM"], self.self.parameters["RH"], self.self.parameters["H_oz"], self.self.parameters["WV"], self.self.parameters["alpha"], self.self.parameters["beta"], self.E_0_res, self.a_oz_res, self.a_ox_res, self.a_wv_res, self.E_dsa_res)
        else:
            E_dsa = self.E_dsa_res

        if len(self.E_dsr_res) == 0:
            E_dsr = downwelling_irradiance.E_dsr(self.wavelengths, self.self.parameters["theta_sun"], self.self.parameters["P"], self.self.parameters["AM"], self.self.parameters["RH"], self.self.parameters["H_oz"], self.self.parameters["WV"], self.E_0_res, self.a_oz_res, self.a_ox_res, self.a_wv_res, self.E_dsr_res)
        else:
            E_dsr = self.E_dsr_res
        ...
        if len(self.E_d_res) == 0:
            E_d = downwelling_irradiance.E_d(E_dd, E_ds, self.self.parameters["f_dd"], self.self.parameters["f_ds"])
        else:
            E_d = self.E_d_res

        dbdcphy = backscattering.db_b_div_dC_phy(wavelengths=self.wavelengths, b_bphy_spec=self.parameters["b_bphy_spec"], b_phy_norm_res=self.b_phy_norm_res)

        def df_div_dC_0():
            dadC0   = absorption.da_div_dC_i(i=0, wavelengths=self.wavelengths, a_i_spec_res=self.a_i_spec_res)
            ...
            df_div_dC_0 = air_water.dbelow2above_div_dp(r_rs=R_rs_water, 
                                                    dr_rs_div_dp=water_alg.dr_rs_shallow_div_dp(r_rs_deep=rrsd,
                                                                                    ...
                                                                                    K_d=Kd,
                                                                                    dK_d_div_dp=attenuation.dK_d_div_dp(dadC0, dbdcphy, ctsp, self.self.parameters["kappa_0"]),
                                                                                    k_uW=kuW,
        ...
        def df_div_dC_1():
            dadC1   = absorption.da_div_dC_i(1, self.wavelengths, self.a_i_spec_res)
            ...
            df_div_dC_1 = air_water.dbelow2above_div_dp(R_rs_water, 
                                                    water_alg.dr_rs_shallow_div_dp(r_rs_deep=rrsd,
                                                                                    ...
                                                                                    K_d=Kd,
                                                                                    dK_d_div_dp=attenuation.dK_d_div_dp(dadC1, dbdcphy, ctsp, self.self.parameters["kappa_0"]),
                                                                                    k_uW=kuW,
        ...
        def df_div_dC_2():
            dadC2   = absorption.da_div_dC_i(2, self.wavelengths, self.a_i_spec_res)
            ...
            df_div_dC_2 = air_water.dbelow2above_div_dp(R_rs_water, 
                                                    water_alg.dr_rs_shallow_div_dp(r_rs_deep=rrsd,
                                                                                    ...
                                                                                    K_d=Kd,
                                                                                    dK_d_div_dp=attenuation.dK_d_div_dp(dadC2, dbdcphy, ctsp, self.self.parameters["kappa_0"]),
                                                                                    k_uW=kuW,
        ...
        def df_div_dC_3():
            dadC3   = absorption.da_div_dC_i(3, self.wavelengths,self. a_i_spec_res)
            ...
            df_div_dC_3 = air_water.dbelow2above_div_dp(R_rs_water, 
                                                    water_alg.dr_rs_shallow_div_dp(r_rs_deep=rrsd,
                                                                                    ...
                                                                                    K_d=Kd,
                                                                                    dK_d_div_dp=attenuation.dK_d_div_dp(dadC3, dbdcphy, ctsp, self.self.parameters["kappa_0"]),
                                                                                    k_uW=kuW,
        ...
        def df_div_dC_4():
            dadC4   = absorption.da_div_dC_i(4, self.wavelengths, self.a_i_spec_res)
            ...
            df_div_dC_4 = air_water.dbelow2above_div_dp(R_rs_water, 
                                                    water_alg.dr_rs_shallow_div_dp(r_rs_deep=rrsd,
                                                                                    ...
                                                                                    K_d=Kd,
                                                                                    dK_d_div_dp=attenuation.dK_d_div_dp(dadC4, dbdcphy, ctsp, self.self.parameters["kappa_0"]),
                                                                                    k_uW=kuW,
        ...
        def df_div_dC_5():
            dadC5   = absorption.da_div_dC_i(5, self.wavelengths, self.a_i_spec_res)
            ...
            df_div_dC_5 = air_water.dbelow2above_div_dp(R_rs_water, 
                                                    water_alg.dr_rs_shallow_div_dp(r_rs_deep=rrsd,
                                                                                    ...
                                                                                    K_d=Kd,
                                                                                    dK_d_div_dp=attenuation.dK_d_div_dp(dadC5, dbdcphy, ctsp, self.self.parameters["kappa_0"]),
                                                                                    k_uW=kuW,
        ...
        def df_div_dC_Y():
            dadCY = absorption.da_div_dC_Y(wavelengths=self.wavelengths, S=self.parameters["S"], lambda_0=self.parameters["lambda_0"], a_Y_N_res=self.a_Y_N_res)
            dbdCY = 0
            ...
            df_div_dC_Y = air_water.dbelow2above_div_dp(R_rs_water, 
                                                    water_alg.dr_rs_shallow_div_dp(r_rs_deep=rrsd,
                                                                                    ...
                                                                                    K_d=Kd,
                                                                                    dK_d_div_dp=attenuation.dK_d_div_dp(dadCY, dbdCY, ctsp, self.self.parameters["kappa_0"]),
                                                                                    k_uW=kuW, 
        ...
        def df_div_dC_X():
            dadCX = absorption.da_div_dC_X(wavelengths=self.wavelengths, lambda_0=self.parameters["lambda_0"], a_NAP_spec_lambda_0=self.parameters["a_NAP_spec_lambda_0"], S_NAP=self.parameters["S_NAP"], a_NAP_N_res=self.a_NAP_N_res)
            dbdCX = backscattering.db_b_div_dC_X(wavelengths=self.wavelengths, b_X_norm_res=self.b_X_norm_res)
        ...
        def df_div_dC_Mie():
            dadCMie = absorption.da_div_dC_Mie(wavelengths=self.wavelengths, lambda_0=self.parameters["lambda_0"], a_NAP_spec_lambda_0=self.parameters["a_NAP_spec_lambda_0"], S_NAP=self.parameters["S_NAP"], a_NAP_N_res=self.a_NAP_N_res)
            dbdCMie = backscattering.db_b_div_dC_Mie(wavelengths=self.wavelengths, n=self.parameters["n"], b_bMie_spec=self.parameters["b_bMie_spec"], lambda_S=self.parameters["lambda_S"], b_bMie_norm_res=self.b_Mie_norm_res)
        ...
        def df_div_df_0():
            df_div_df_0 = air_water.dbelow2above_div_dp(R_rs_water,
                                                    water_alg.dr_rs_shallow_div_dp(r_rs_deep=rrsd,
                                                                                    ...
                                                                                    dk_uW_div_dp=0,
                                                                                    r_rs_b=Rrsb,
                                                                                    d_r_rs_b_div_dp=bottom_reflectance.dR_rs_b_div_df_i(0, wavelengths=self.wavelengths, R_i_b_res=self.R_i_b_res),
                                                                                    k_uB=kuB,
                                                                                    dk_uB_div_dp=0,
                                                                                    zB=self.parameters["zB"]
                                                    )
        ...
        def df_div_df_1():
            df_div_df_1 = air_water.dbelow2above_div_dp(R_rs_water,
                                                    water_alg.dr_rs_shallow_div_dp(r_rs_deep=rrsd,
                                                                                    ...
                                                                                    dk_uW_div_dp=0,
                                                                                    r_rs_b=Rrsb,
                                                                                    d_r_rs_b_div_dp=bottom_reflectance.dR_rs_b_div_df_i(1, wavelengths=self.wavelengths, R_i_b_res=self.R_i_b_res),
                                                                                    k_uB=kuB,
                                                                                    dk_uB_div_dp=0,
                                                                                    zB=self.parameters["zB"]
                                                    )
        ...
        def df_div_df_2():
            df_div_df_2 = air_water.dbelow2above_div_dp(R_rs_water,
                                                    water_alg.dr_rs_shallow_div_dp(r_rs_deep=rrsd,
                                                                                    ...
                                                                                    dk_uW_div_dp=0,
                                                                                    r_rs_b=Rrsb,
                                                                                    d_r_rs_b_div_dp=bottom_reflectance.dR_rs_b_div_df_i(2, wavelengths=self.wavelengths, R_i_b_res=self.R_i_b_res),
                                                                                    k_uB=kuB,
                                                                                    dk_uB_div_dp=0,
                                                                                    zB=self.parameters["zB"]
                                                    )
        ...
        def df_div_df_3():
            df_div_df_3 = air_water.dbelow2above_div_dp(R_rs_water,
                                                    water_alg.dr_rs_shallow_div_dp(r_rs_deep=rrsd,
                                                                                    ...
                                                                                    dk_uW_div_dp=0,
                                                                                    r_rs_b=Rrsb,
                                                                                    d_r_rs_b_div_dp=bottom_reflectance.dR_rs_b_div_df_i(3, wavelengths=self.wavelengths, R_i_b_res=self.R_i_b_res),
                                                                                    k_uB=kuB,
                                                                                    dk_uB_div_dp=0,
                                                                                    zB=self.parameters["zB"]
                                                    )
        ...
        def df_div_df_4():
            df_div_df_4 = air_water.dbelow2above_div_dp(R_rs_water,
                                                    water_alg.dr_rs_shallow_div_dp(r_rs_deep=rrsd,
                                                                                    ...
                                                                                    dk_uW_div_dp=0,
                                                                                    r_rs_b=Rrsb,
                                                                                    d_r_rs_b_div_dp=bottom_reflectance.dR_rs_b_div_df_i(4, wavelengths=self.wavelengths, R_i_b_res=self.R_i_b_res),
                                                                                    k_uB=kuB,
                                                                                    dk_uB_div_dp=0,
                                                                                    zB=self.parameters["zB"]
                                                    )
        ...
        def df_div_df_5():
            df_div_df_5 = air_water.dbelow2above_div_dp(R_rs_water,
                                                    water_alg.dr_rs_shallow_div_dp(r_rs_deep=rrsd,
                                                                                    ...
                                                                                    dk_uW_div_dp=0,
                                                                                    r_rs_b=Rrsb,
                                                                                    d_r_rs_b_div_dp=bottom_reflectance.dR_rs_b_div_df_i(5, wavelengths=self.wavelengths, R_i_b_res=self.R_i_b_res),
                                                                                    k_uB=kuB,
                                                                                    dk_uB_div_dp=0,
                                                                                    zB=self.parameters["zB"]
                                                    )
        ...
        if self.parameters["fit_surface"].value:
            def df_div_dg_dd():
            ...
            zero = np.zeros_like(self.wavelengths)
    ...
    def invert(self,
        ...
        ret_params      = self.parameters.copy()
        fit_params      = []
        fit_bounds      = [[],[]]
        fit_param_names = []
        ...
        userfun  = self.get_fun_shim(self.get_residuals(self.forward, self.R_rs, self.weights), self.wavelengths, fit_param_names, ret_params)
        userdfun = self.get_dfun_shim(self.dfun, self.wavelengths, fit_param_names, ret_params)

        if analytical:
            analytical_fit = least_squares(userfun, fit_params, bounds=fit_bounds, jac=userdfun,
                                        kwargs={
                                            "a_res": self.a_res,
                                            "b_b_res": self.b_b_res,
                                            "a_i_spec_res": self.a_i_spec_res,
                                            "a_w_res": self.a_w_res,
                                            "a_Y_N_res": self.a_Y_N_res,
                                            "a_NAP_N_res": self.a_NAP_N_res,
                                            ...
                                            "b_X_norm_res": self.b_X_norm_res,
                                            "b_Mie_norm_res": self.b_Mie_norm_res,
                                            "R_i_b_res": self.R_i_b_res,
                                            "da_W_div_dT_res": self.da_W_div_dT_res,
                                            "E_0_res": self.E_0_res,
                                            "a_oz_res": self.a_oz_res,
                                            "a_ox_res": self.a_ox_res,
                                            "a_wv_res": self.a_wv_res,
                                            "E_dd_res": self.E_dd_res,
                                            "E_dsa_res": self.E_dsa_res,
                                            "E_dsr_res": self.E_dsr_res,
                                            "E_d_res": self.E_d_res,
                                            "E_ds_res": self.E_ds_res,
                                            "n2_res": self.n2_res,
                                            "Ls_Ed": self.Ls_Ed,
                                        },
                                        max_nfev=max_nfev)
