...
# Here we try to implement a simple Bayesian add-on to the inversion following the methodology described in Erickson et al. (2023) [1] and the corresponding GitHub repo [2].
# 
#   [1] Erickson et al. (2023): Bayesian approach to a generalized inherent optical property model. [10.1364/oe.486581]
#   [2] Erickson (2023): zacharykerickson/pygiop. [10.5281/zenodo.7869394]
#
# Implementation requires some new functions and some existing functions to be changed.
...
def calculate_full_covariance_matrix(covar,jac):
    """
    Uncertainty propagation using the covariance matrix of measurements and the Jacobian matrix of parameters.
    This formulation assumes that the uncertainties are normally distributed and utilizes the linear approximation of error propagation.
    It assumes that the Jacobian is a good approximation of the sensitivity of the output variables, which is problematic if the system is highly nonlinear.
    We use the Moore-Penrose pseudo-inverse of the Jacobian to handle non-invertible or non-square Jacobians, which might be appropriate for over- or under-determined systems.
    
    Args:
        covar: Covariance matrix of measurements
        jac: Jacobian matrix of parameters

    Returns:
        covar_prop_pinv: Full covariance matrix
                         This matrix captures the covariances and variances associated with the propagated uncertainties in the output variables 
                         due to uncertainties in the input variables. It fully describes the relationship and uncertainties in the predicted values 
                         of the output variables based on the uncertainties in the input parameters, considering the linear approximation of the 
                         error propagation.
    """
    # Moore-Penrose pseudo inverse of Jacobian matrix using its singular-value decomposition (SVD; https://numpy.org/doc/stable/reference/generated/numpy.linalg.pinv.html)
    # Using the pseudo-inverse with a regularization parameter is appropriate when dealing with potentially non-invertible or ill-conditioned Jacobians and adds numerical stability to the computation.
    pinv_jac = np.linalg.pinv(jac,rcond=1e-5)
    # Propagate uncertainty using pseudo-inverse Jacobian. 
    # This is the full covariance matrix.
    covar_prop_pinv = np.dot(pinv_jac,np.dot(covar,pinv_jac.T))

    return covar_prop_pinv


def calculate_posterior_covariance_matrix(r_rs_cov, prior_cov, jac):
    """
    Calculate posterior covariance matrix.
    The posterior covariance matrix is a matrix that describes the uncertainty or variability in the estimated parameters based on the observed data and prior information. 

    Args:
        r_rs_cov: covariance matrix of the spectra
        prior_cov: covariance matrix of the priors
        jac: Jacobian matrix for selected fit parameters
        
    Returns:
        Posterior covariance matrix
    """
    posterior_covariance = np.linalg.inv(np.dot(jac.T, np.dot(np.linalg.inv(r_rs_cov), jac)) + np.linalg.inv(prior_cov))
    return posterior_covariance


def calculate_parameter_uncertainties(r_rs_cov, prior_cov, jac):
    """
    Calculate the square root of the diagonal elements of the posterior covariance matrix, which provides the standard deviations of the parameter distributions as a measure of parameter uncertainties.
    ...
    Returns:
        Standard deviations of the parameters as a measure of parameter uncertainties
    """
    posterior_covariance = calculate_posterior_covariance_matrix(r_rs_cov, prior_cov, jac)
    parameter_uncertainties = np.sqrt(np.diag(posterior_covariance))
    return parameter_uncertainties


def calculate_bayesian_residual(r_rs_obs, r_rs_sim, prior_means, parameter_values, jac, measurement_uncertainty):
    """
    Compute residual for Bayesian approach following [1,2]

    [1] Erickson et al. (2023): Bayesian approach to a generalized inherent optical property model. [10.1364/oe.486581]
    [2] Erickson (2023): zacharykerickson/pygiop. [10.5281/zenodo.7869394]

    Args:
        r_rs_obs: observed above-water radiance reflectance [sr-1]
        r_rs_sim: simulated above-water radiance reflectance [sr-1]
        prior_means: prior for fit parameters (np.array)
        parameter_values: estimated fit parameters (np.array)
        jac: Jacobian matrix for selected fit parameters
        measurement_uncertainty: sensor noise matrix of shape (nbands,nbands) with band-wise SNR on the diagonal
        
    Returns:
        Combined residual vector consisting of the residuals of the spectra and the priors
    """

    r_rs_error = r_rs_sim - r_rs_obs
    prior_error = parameter_values - prior_means

    # !!! not sure if this is a proper definition of r_rs_cov. Shouldnt r_rs_error be involved, too? !!!
    r_rs_obs_uncertainty = np.dot(measurement_uncertainty, r_rs_obs)
    # Generate measurement error covariance matrix due to instrument noise (covRrs in [2], S_R in the [1])
    r_rs_cov = np.diag(r_rs_obs_uncertainty**2) 

    # Generate prior covariance matrix using the Jacobian and noise covariance matrix (cov_prior in [2], S_R in the [1])
    prior_cov = calculate_full_covariance_matrix(r_rs_cov, jac)  

    # Combine the residuals of r_rs and priors. This is Eq. 10 in [1].
    r_rs_residual = (r_rs_error**2.)/np.diag(r_rs_cov)
    prior_residual = (prior_error**2.)/np.diag(prior_cov)
    combined_residual = np.concatenate((r_rs_residual, prior_residual))

    # Calculate parameter uncertainties
    parameter_uncertainties = calculate_parameter_uncertainties(r_rs_cov, prior_cov, jac)

    return combined_residual, parameter_uncertainties, prior_cov


########################################################
############ BELOW IS THE OLF STUFF ####################
########################################################

# !!! This function is no longer necessary or needs to be changed to use the combined_residual !!!
...
# # !!! This function needs to be changed to use the combined_residual !!!
