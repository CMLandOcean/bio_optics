...
def forward(parameters,
    ...
    a_sim = a_res if len(a_res) > 0 else absorption.a(
        ...
        S=parameters["S"], S_NAP=parameters["S_NAP"],
        a_NAP_spec_lambda_0=parameters["a_NAP_spec_lambda_0"],
        lambda_0=parameters["lambda_0"], K=parameters["K"],
        wavelengths=wavelengths, T_W=parameters["T_W"], T_W_0=parameters["T_W_0"],
    ...
    bb_sim = bb_res if len(bb_res) > 0 else backscattering.bb(
        ...
        wavelengths=wavelengths, fresh=parameters["fresh"],
        bb_phy_spec=parameters["b_bphy_spec"], bb_Mie_spec=parameters["b_bMie_spec"],
        bb_X_spec=parameters["b_bX_spec"], b_X_norm_factor=parameters["b_bX_norm_factor"],
        lambda_S=parameters["lambda_S"], n=parameters["n"],
        bb_w_res=bb_w_res, b_phy_norm_res=b_phy_norm_res,
        b_X_norm_res=b_X_norm_res, b_Mie_norm_res=b_Mie_norm_res)

    Rrs_b = bottom_reflectance.Rrs_b(
        parameters["f_0"], parameters["f_1"], parameters["f_2"],
        parameters["f_3"], parameters["f_4"], parameters["f_5"],
        B_0=parameters["B_0"], B_1=parameters["B_1"], B_2=parameters["B_2"],
        B_3=parameters["B_3"], B_4=parameters["B_4"], B_5=parameters["B_5"],
        wavelengths=wavelengths, R_b_i_res=R_b_i_res)

    ob  = attenuation.omega_b(a_sim, bb_sim)
    frs = f_rs(omega_b=ob, cos_t_sun_p=ctsp, cos_t_view_p=ctvp)
    rrsd = rrs_deep(f_rs=frs, omega_b=ob)
    ...
    return air_water.below2above(rrs_shallow(rrs_deep=rrsd, Kd=Kd, ku_w=kuW,
                                             zB=parameters["zB"], Rrs_b=Rrs_b, ku_b=kuB))
