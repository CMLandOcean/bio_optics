...
from bio_optics.models import hereon_c2rcc
...
wavelengths= np.arange(443, 560, 1) # Reference wavelength C2RCC: 443nm
...
outpath = "D:\Documents\projects\EO-TDL\c2rcc_like\\"
path = "Y:\\related\EO-TDL\Simulations\\"

fnames_ = os.listdir(path)
iopFnames = [fn for fn in fnames_ if fn.startswith('iop_')] # "Y:\related\EO-TDL\Simulations\iop_Training_PACE2Model_20250512_HEREON.txt"


paramDF = pd.read_csv(path + iopFnames[0], header=0, sep='\t') ## as a test, use only nrows = 300
colList = [a for a in paramDF.columns.values if not 'Unnamed' in a]
paramDF = paramDF[colList]
...
if np.sum(ID)>0:
    ...
    def simulate_chunk(
        ...
        for i in np.arange(paramDF_chunk.shape[0]):
            ...
            if i == 0:
                IOP = hereon_c2rcc.forward_c2rcc_IOPs(parameters=params,
                ...
                IOP = np.vstack((IOP, hereon_c2rcc.forward_c2rcc_IOPs(parameters=params,
                                                               wavelengths=wavelengths,
                                                               a_md_spec_res=a_md_spec_res,
                                                               a_bd_spec_res=a_bd_spec_res,
                                                               a_w_res=a_w_res,
    ...
    IOP_c2rcc_sim = pd.DataFrame(processed_data, columns=['apig', 'adet', 'agelb', 'bpart'])
    IOP_c2rcc_sim.to_csv(outpath + "IOPc2rcc_" + AlgaeGroupType + "_simulation_" + iopFnames[0], header=True, sep='\t', index=False)