...
def processor_bioOptics_hyperspectral_EnMAPExtracts(
        versionAC = '',
        regionName = '',
        AlgaeGroupType = '',
    ...
    if not outpath is None:
        outDF.to_csv(outpath + "inverted_IOP_bio_optics_"+AlgaeGroupType+"_" + regionName + '_'+ addFilePart  + "_V"+str(int(Ngroups)) +
                 "AH_"+groupStr+"_restrict"+addFilePart2+".txt",
                 sep='\t', header=True, index=False)
        R_rs_sim.to_csv(outpath + "inverted_Rrs_bio_optics_"+AlgaeGroupType+"_" + regionName +'_'+ addFilePart + "_V"+str(int(Ngroups)) +
                 "AH_"+groupStr+"_restrict"+addFilePart2+".txt",
                    sep='\t', header=True, index=False)

    return None

def read_EnMAP_Extracts(path, fname, maxWL=900.):
    d = pd.read_csv(path + fname, header=0, sep='\t')
    wlstr = [b for b in d.columns.values if
             not b in ['idx', 'idy', 'lon', 'lat', 'OWT', 'date', 'QWIP', 'AVW', 'AVW_', 'AVW_QWIP', 'membSum', 'Area', 'NDI']]
...
processingDict_Oder = {
    ...
    #'C_5': setDictValues(0,0,200), # coccolith., or gold algae
...
dataType = 'EnMAPExtracts' # 'Sim'
AlgaeGroupType = 'Standardv2' #'Standardv2' # 'Standardv3' # 'NSSummerBloomsv3' Standardv2, 'HEREONgold'

# maxWavelength= 750.
maxWavelength = 900.

path = "D:\Documents\projects\EnsAD\EnMAP\L2A_Land\\v010502\Fluss_Elbe_Oder\\extracts\\"
outpath = "D:\Documents\projects\EnsAD\EnMAP\L2A_Land\\v010502\Fluss_Elbe_Oder\\inversion\\"
ElbeDateList = ['20240606T105228', '20240811T110300', '20240930T105918', '20241004T110257']
OderDateList = ['20220720T104147', '20220720T104151', '20230624T103126', '20240309T104508', '20240309T104512',
                '20240814T104130', '20240814T104134', '20240921T102708',  '20240921T102713' , '20241011T104523']
SpatialFilter = [False, False, True, True, True,
                 False, True, False, True, True]
fnames = os.listdir(path)
fnamesL = []
for spf, oderDate in zip(SpatialFilter, OderDateList):
    if not spf:
        [fnamesL.append(fn) for fn in fnames if oderDate in fn]
print(fnamesL)

## read file for wavelengths
## read EnMAP
fname = fnamesL[0]
# addFilePart = fname.split('____L2A')[1].split('_')[1]
rrs, wavelengths, wlstr = read_EnMAP_Extracts(path, fname)
print(rrs.shape)

# ## exclude invalid spectrum
# IDvalid = np.sum(np.isnan(rrs.values), axis=1) == 0
# print(np.sum(IDvalid), IDvalid.shape)
# rrs = rrs.loc[IDvalid,:]
...
for fname in fnamesL:
    ...
    processor_bioOptics_hyperspectral_EnMAPExtracts(regionName = regionName,
