...
def qaa_cdom(Rrs, 
    ...
    !!! Note that this implementation requires Rrs and wavelengths to contain only the relevant bands !!!
    ...
    Args:
        Rrs: remote sensing reflectance [sr-1] for selected bands (lambdas)
        wavelengths: corresponding wavelengths [nm]
    ...
    # Step 2 [1]
    # !!! Note that in [1] chi is computed from Rrs, while r_rs is used in [2,3]. This is likely a typo in the paper but tests showed that the influence on chi is small, so we stuck with the version described in [1].
