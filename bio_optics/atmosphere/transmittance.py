...
def M(theta_sun=np.radians(30), a=0.50572, b=6.07995, c=1.6364):
    """
    Atmospheric path length after Kasten and Young (1989) [1] as used in WASI [2].

    [1] Kasten and Young (1989): Revised optical air mass tables and approximation formula.
...
def M_cor(theta_sun=np.radians(30), P=1013.25):
    """
    Atmospheric path length corrected for non-standard atmospheric pressure [1].
...
def M_oz(theta_sun=np.radians(30)):
    ...
    Returns:
        M_oz: atmospheric path length for ozone [dimensionless]
    """
...
def compute_beta(V=12, H_a=1):
    """
    Turbidity coefficient of aerosol optical thickness computed from visibility and aerosol scale height [1].

    [1] Gege, P. (2021): The Water Colour Simulator WASI. User manual for WASI version 6.

    Args:
        V: horizontal visibility [km], default: 12
        H_a: aerosol scale height [km], default: 1

    Returns:
        beta: turbidity coefficient of aerosol optical thickness [dimensionless]
    """
    beta = 3.91 * H_a / V
    return beta
...
def tau_a(wavelengths=np.arange(400,800), lambda_a=550, alpha=1.317, beta=0.2606, V=None, H_a=None):
    """
    Aerosol optical thickness after Gege (2021) [1].
    The turbidity coefficient beta can optionally be computed from visibility V and aerosol scale height H_a.
    ...
    Args:
        ...
        V: horizontal visibility [km]; if provided together with H_a, beta is computed from these, default: None
        H_a: aerosol scale height [km]; used together with V to compute beta, default: None

    Returns:
        tau_a: aerosol optical thickness [dimensionless]
    """
...
def omega_a(AM=5, RH=80):
    """
    Aerosol single scattering albedo after Gege (2021) [1].
...
def F_a(theta_sun=np.radians(30), alpha=1.317):
    """
    Aerosol forward scattering probability after Gege (2021) [1].
...
def T_r(wavelengths=np.arange(400,800), theta_sun=np.radians(30), P=1013.25):
    """
    Rayleigh scattering transmittance after Gege (2021) [1].
    Note: wavelengths are converted to micrometres internally.
    ...
    Returns:
        T_r: Rayleigh scattering transmittance [dimensionless]
    """
...
def T_aa(wavelengths=np.arange(400,800), theta_sun=np.radians(30), AM=5, RH=80, lambda_a=550, alpha=1.317, beta=0.2606):
    """
    Aerosol absorption transmittance after Gege (2021) [1].
    ...
    Returns:
        T_aa: aerosol absorption transmittance [dimensionless]
    """
...
def T_as(wavelengths=np.arange(400,800), theta_sun=np.radians(30), AM=5, RH=80, lambda_a=550, alpha=1.317, beta=0.2606):
    """
    Aerosol scattering transmittance after Gege (2021) [1].
    ...
    Returns:
        T_as: aerosol scattering transmittance [dimensionless]
    """
...
def T_oz(wavelengths=np.arange(400,800), theta_sun=np.radians(30), H_oz=0.381, a_oz_res=None):
    ...
    Returns:
        T_oz: ozone absorption transmittance [dimensionless]
    """
...
def T_ox(wavelengths=np.arange(400,800), theta_sun=np.radians(30), P=1013.25, a_ox_res=None):
    ...
    Returns:
        T_ox: oxygen absorption transmittance [dimensionless]
    """
...
def T_wv(wavelengths=np.arange(400,800), theta_sun=np.radians(30), WV=2.5, a_wv_res=None):
    ...
    Returns:
        T_wv: water vapour absorption transmittance [dimensionless]
    """
